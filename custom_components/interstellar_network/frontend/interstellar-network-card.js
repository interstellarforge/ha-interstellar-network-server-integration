const INTERSTELLAR_CARD_VERSION = "0.5.2";
...
class InterstellarNetworkCard extends HTMLElement {
  ...
  _model(){
    ...
    const all=[...groups.entries()].map(([id,entities])=>{
      ...
      return {id,name,roles,entities,data,control,policy,docker,wake,online,waking,access,status:online?(warning?"problem":"healthy"):"offline",capabilities,manageableServices:new Set(manageableServices),manageableContainers:new Set(manageableContainers)};
    });
  ...
  _detailed(server){
    ...
    return `<article class="detailed-server ${server.status} ${this._selectedServer===server.id?"selected":""}" data-server="${ESC(server.id)}"><header class="server-head"><div><div class="server-title"><h3>${ESC(server.name)}</h3><span class="status ${server.status}">${server.online?"●":"○"} ${ESC(server.waking?"Waking…":server.status)}</span><span class="access" title="${ESC(server.access.headline)}">${ESC(server.access.badge)}</span></div><div class="muted">${ESC(server.roles.join(" · ")||"general")} · Last seen ${ESC(AGE(d.timestamp_utc))}</div>${server.online&&!server.access.managed?`<div class="access-note">${ESC(server.access.headline)}</div>`:""}</div><div class="head-metrics"><span>CPU ${ESC(d.cpu?.used_percent??"—")}%</span><span>RAM ${ESC(d.memory?.used_percent??"—")}%</span><span>Disk ${ESC(d.disk_root?.used_percent??"—")}%</span></div></header><div class="server-body">${!server.online?'<div class="stale-banner">Offline · values below are last reported data.</div>':""}${badges.length?`<div class="badges">${badges.map(x=>`<span>${ESC(x)}</span>`).join("")}</div>`:""}${resources}${this._system(server)}${this._updates(server)}${this._services(server)}${this._docker(server)}${this._network(server)}${this._disks(server)}${temperatures}${actions}${this._manage(server)}</div></article>`;
  }
