const INTERSTELLAR_CARD_VERSION = "0.5.3";
...
const CONTROL_CAPABILITY = "interstellarnetwork.nl/cap/server-control";
// Why control is unavailable, in the server's own terms. `reachable` means Home
// Assistant spoke to the control API and was refused, which is a very different
// problem from a service that is not running.
const CONTROL_ERRORS = {
  tailscale_capability_missing:{label:"Authorization required",headline:"Control API reachable · Tailscale control capability missing",reachable:true,
    detail:`Home Assistant can reach the control API, but the required Tailscale app capability has not been granted.`,capability:true},
  unauthorized:{label:"Authorization required",headline:"Control API reachable · Home Assistant is not authenticated",reachable:true,
    detail:"The control API rejected Home Assistant's identity.",capability:true},
  not_configured:{label:"Not configured",headline:"No control URL configured",
    detail:"Add the control URL in the integration options to enable management."},
  not_found:{label:"Not reachable",headline:"Control route not found",reachable:true,
    detail:"The control URL responded but has no control API on it. The canonical URL is the server's MagicDNS name on port 8443."},
  connection_refused:{label:"Not reachable",headline:"Connection refused",detail:"Nothing accepted the connection on the control URL."},
  dns_error:{label:"Not reachable",headline:"Server name could not be resolved",detail:"Check the control URL host name."},
  timeout:{label:"Not reachable",headline:"Control API timed out",detail:"The control API did not respond in time."},
  tls_error:{label:"Not reachable",headline:"TLS verification failed",detail:"The control URL's certificate could not be verified."},
  service_unavailable:{label:"Unavailable",headline:"Control service reported itself unavailable",reachable:true,
    detail:"The control API is running but could not read its own state."},
  server_error:{label:"Unavailable",headline:"Control API returned an error",reachable:true,detail:"The control API reported an internal error."},
  invalid_response:{label:"Unavailable",headline:"Unexpected control response",reachable:true,detail:"The control URL did not return a valid control API response."},
};
const ACCESS = (control,available) => {
  if(available)return {managed:true,badge:"🔐 Managed",label:"Managed",headline:"",detail:"",capability:false};
  const info=CONTROL_ERRORS[control.error_code]||null;
  const detail=info?.detail||control.unavailable_reason||"";
  return {managed:false,badge:"👁 Read-only",label:info?.label||"Read-only",
    headline:info?.headline||control.unavailable_reason||"Control is unavailable",
    detail,reachable:!!info?.reachable,capability:!!info?.capability};
};
// Every state, badge and meter the card shows names a condition someone has to
// act on. A label alone ("OOM", "Inodes", "inactive") only helps a reader who
// already knows it, so each one carries its own explanation on hover.
const SERVICE_STATE_HELP = {
  active:"The unit is running.",running:"The container is running.",
  inactive:"The unit is loaded but not running: it was stopped, or it never started.",
  failed:"The unit exited with an error and systemd stopped trying to restart it. `systemctl status` on the server has the exit code and the last log lines.",
  activating:"The unit is still starting up.",deactivating:"The unit is shutting down.",
  masked:"The unit is masked, so systemd refuses to start it at all.",
  "not-found":"No such unit on this server. It is expected by the server's role but is not installed.",
  unknown:"The health agent could not read this unit's state.",
  healthy:"The container's own health check is passing.",
  unhealthy:"The container is running, but its own health check is failing.",
  starting:"The container is running; its health check has not passed yet.",
  exited:"The container has stopped.",restarting:"The container is restarting, which usually means it is crash-looping.",
  paused:"The container is paused.",created:"The container was created but never started.",
  dead:"The container is dead and has to be removed before it can run again.",
};
const BAR_HELP = {
  cpu:"Share of the server's total CPU capacity in use right now.",
  ram:"Share of physical memory in use. Linux also uses free memory as cache, which is not counted here.",
  "root disk":"Share of the root filesystem's capacity in use. Above 90% services begin failing to write.",
  usage:"Share of this filesystem's capacity in use.",
  inodes:"Share of the filesystem's inodes in use. Every file, directory and link takes one, so a filesystem can run out of inodes while it still reports free space.",
};
const CHIP_TITLE = (name,state,expected,active) => {
  const shown=state?String(state):"not reported",help=SERVICE_STATE_HELP[NORM(state)]||"";
  const head=`${SERVICE_LABEL(name)} \u00b7 ${shown}${expected?" \u00b7 expected to be running on this server":""}`;
  return help?`${head}\n${help}`:head;
};
// The single source of truth for "problem": the status pill, the hover text and
// the detailed badges all read this list, so they can never disagree.
const PROBLEMS = (data,docker) => {
  const out=[],down=LIST(data.service_policy?.problems),units=LIST(data.system?.failed_units).map(u=>u.unit).filter(Boolean);
  const unitCount=units.length||NUM(data.system?.failed_systemd_units)||0,security=NUM(data.updates?.pending_security)||0;
  const disk=NUM(data.disk_root?.used_percent),oom=NUM(data.system?.oom_kills_since_boot)||0;
  const unhealthy=LIST(docker.containers).filter(c=>c.health==="unhealthy").map(c=>c.name).filter(Boolean);
  if(down.length)out.push({key:"services",label:"Service down",
    detail:`A service this server's role requires is not running. Not active: ${down.map(p=>`${SERVICE_LABEL(p.service)} (${p.state||"unknown"})`).join(", ")}.`});
  if(unitCount>0)out.push({key:"units",label:`${unitCount} failed unit${unitCount===1?"":"s"}`,
    detail:`systemd reports ${unitCount} unit${unitCount===1?"":"s"} in a failed state${units.length?`: ${units.join(", ")}`:""}. They exited with an error and were not restarted.`});
  if(security>0)out.push({key:"security",label:`${security} security`,
    detail:`${security} pending package update${security===1?"":"s"} ${security===1?"is":"are"} marked as a security fix${security===1?"":"es"}. Install ${security===1?"it":"them"} from Manage \u2192 Install security updates.`});
  if(disk!==null&&disk>=90)out.push({key:"disk",label:"Disk warning",
    detail:`The root filesystem is ${disk.toFixed(0)}% full. Above 90% services start failing to write, and some stop.`});
  if(oom>0)out.push({key:"oom",label:"OOM",
    detail:`Out of memory: the kernel killed ${oom} process${oom===1?"":"es"} since boot because the server ran out of RAM. Whatever was killed did not exit on its own \u2014 check memory use and container memory limits.`});
  if(unhealthy.length)out.push({key:"containers",label:`${unhealthy.length} container${unhealthy.length===1?"":"s"} unhealthy`,
    detail:`Docker reports ${unhealthy.length===1?"this container":"these containers"} running but failing ${unhealthy.length===1?"its":"their"} own health check: ${unhealthy.join(", ")}.`});
  if(data.system?.reboot_required)out.push({key:"reboot",label:"Reboot required",
    detail:"An installed update replaced a kernel or core library that is still running from the old version. The server keeps working, but the update only takes effect after a reboot."});
  return out;
};
const STATUS_TITLE = (server) => {
  if(server.waking)return "A wake packet was sent. The server has not reported back to the health agent yet.";
  if(!server.online)return `No data is arriving from the health agent. Everything shown is the last reported data, from ${AGE(server.data.timestamp_utc)}.`;
  if(!server.problems.length)return "No problems reported: expected services running, no failed units, no pending security updates, root disk below 90%, no reboot required.";
  return `${server.problems.length} problem${server.problems.length===1?"":"s"}:\n`+server.problems.map(p=>`\u2022 ${p.label} \u2014 ${p.detail}`).join("\n");
};
// Pending updates are routine; pending security updates are not. The footer says
// which of the two it is with colour, not only with a number.
const UPDATE_LEVEL = (up) => {
  const security=NUM(up?.pending_security),pending=NUM(up?.pending);
  if(security===null&&pending===null)return {level:"unknown",title:"The health agent has not reported package update counts for this server yet."};
  if((security||0)>0)return {level:"critical",title:`${security} of the ${pending??security} pending update${(pending??security)===1?"":"s"} fix${security===1?"es":""} ${security===1?"a known security vulnerability":"known security vulnerabilities"}. Install ${security===1?"it":"them"} from Manage \u2192 Install security updates.`};
  if((pending||0)>0)return {level:"warning",title:`${pending} package update${pending===1?"":"s"} pending, none of them security fixes.`};
  return {level:"ok",title:"Fully up to date: no pending package updates."};
};
...
class InterstellarNetworkCard extends HTMLElement {
  ...
  _model(){
    ...
    const all=[...groups.entries()].map(([id,entities])=>{
      ...
      const problems=PROBLEMS(data,docker),warning=problems.length>0;
      ...
      const access=ACCESS(control,available);
      return {id,name,roles,entities,data,control,policy,docker,wake,online,waking,access,problems,status:online?(warning?"problem":"healthy"):"offline",capabilities,manageableServices:new Set(manageableServices),manageableContainers:new Set(manageableContainers)};
    });
  ...
  _bar(label,value,sub=""){const n=NUM(value),width=n===null?0:Math.min(100,Math.max(0,n)),help=BAR_HELP[NORM(label)]||"";return `<div class="bar-row"><div class="bar-head"><span ${help?`class="help" title="${ESC(help)}"`:""}>${ESC(label)}</span><strong>${n===null?"Not reported":`${n.toFixed(0)}%`}</strong></div><div class="track" role="meter" aria-label="${ESC(label)}" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${width}"><div class="fill ${n>=90?"critical":n>=75?"warning":""}" style="width:${width}%"></div></div>${sub?`<small>${ESC(sub)}</small>`:""}</div>`;}
  _chip(name,state,expected=false){const active=["active","running","healthy","on"].includes(NORM(state)),problem=expected&&!active;return `<span class="service-chip ${active?"active":problem?"problem":"inactive"}" title="${ESC(CHIP_TITLE(name,state,expected,active))}"><span aria-hidden="true">${active?"●":"○"}</span> ${ESC(SERVICE_LABEL(name))}</span>`;}
  ...
  _compact(server){
    ...
    const statusLabel=server.waking?"Waking…":server.status[0].toUpperCase()+server.status.slice(1),updates=UPDATE_LEVEL(d.updates);
    return `<button class="compact-server ${server.status}" data-select-server="${ESC(server.id)}" aria-label="Open details for ${ESC(server.name)}"><div class="compact-head"><strong>${ESC(server.name)}</strong><span class="status help ${server.status}" title="${ESC(STATUS_TITLE(server))}">${offline?"○":"●"} ${ESC(statusLabel)}</span></div><div class="compact-meta"><span>${ESC(server.roles.join(" · ")||"general")}</span><span title="${ESC(server.access.headline)}">${ESC(server.access.badge)}</span></div>${offline?`<div class="stale-line">Last seen ${ESC(AGE(d.timestamp_utc))}</div>`:""}${!offline&&server.access.capability?'<div class="stale-line">Control capability missing</div>':""}<div class="compact-resources"><div><span>CPU</span><strong>${NUM(cpu)===null?"—":`${NUM(cpu).toFixed(0)}%`}</strong></div><div><span>RAM</span><strong>${NUM(ram)===null?"—":`${NUM(ram).toFixed(0)}%`}</strong></div><div><span>Disk</span><strong>${NUM(disk)===null?"—":`${NUM(disk).toFixed(0)}%${offline?"*":""}`}</strong></div></div>${this._compactServices(server)}<div class="compact-footer">${this.config.compact.show_updates?`<span class="updates-line help ${ESC(updates.level)}" title="${ESC(updates.title)}">Updates ${ESC(d.updates?.pending??"—")} · Security ${ESC(d.updates?.pending_security??"—")}</span>`:""}${this.config.compact.show_uptime?`<span>Uptime ${ESC(UPTIME(d.host?.uptime_seconds))}</span>`:""}</div>${offline&&NUM(disk)!==null?"<small>* last known</small>":""}</button>`;
  }
  ...
  _system(server){
    ...
    // Control-sourced values are absent whenever control is unavailable. Say so
    // explicitly instead of showing a bare dash that looks like a missing field.
    const blocked=server.access.managed?null:server.access.label;
    const fromControl=(value,when=blocked)=>value??(when||null);
    const rows=[["Hostname",server.name],["Roles",server.roles.join(" · ")||"general"],["Boot time",DATE_TIME(h.boot_time_utc||c.boot_time_utc)],["Uptime",UPTIME(h.uptime_seconds)],["OS",h.os],["Kernel",h.kernel],["Architecture",h.architecture],["Virtualization",h.virtualization],["CPU",d.cpu?.model?`${d.cpu.model} · ${REPORTED(d.cpu?.count)} cores`:null],["Health agent",d.agent_version],["Toolbox",fromControl(c.toolbox_version)],["Control",fromControl(c.version||c.control_version)],["Tailscale",c.tailscale_version||d.network?.tailscale?.version],["Last seen",AGE(d.timestamp_utc)],["Failed units",failed],["OOM kills",sys.oom_kills_since_boot??0],["Last reboot",reboot?`${DATE_TIME(reboot.timestamp)} · ${REPORTED(reboot.status)}`:fromControl(null)],["Reboot duration",duration!==null&&duration!==undefined?UPTIME(duration):fromControl(null)]];
  ...
  _manage(server){
    ...
    const readonly=!managed?`<div class="read-only"><strong>Read-only</strong><p>${ESC(access.headline)}</p>${access.detail?`<p>${ESC(access.detail)}</p>`:""}${access.capability?`<p class="capability">Required capability:<code>${ESC(CONTROL_CAPABILITY)}</code></p>`:""}</div>`:"";
    return this._section(server,"manage","Manage",`${readonly}${quick}${updates}${interstellar}${power}${wake}`);
  }

  _detailed(server){
    const d=server.data,badges=server.problems;
    ...
    return `<article class="detailed-server ${server.status} ${this._selectedServer===server.id?"selected":""}" data-server="${ESC(server.id)}"><header class="server-head"><div><div class="server-title"><h3>${ESC(server.name)}</h3><span class="status help ${server.status}" title="${ESC(STATUS_TITLE(server))}">${server.online?"●":"○"} ${ESC(server.waking?"Waking…":server.status)}</span><span class="access" title="${ESC(server.access.headline)}">${ESC(server.access.badge)}</span></div><div class="muted">${ESC(server.roles.join(" · ")||"general")} · Last seen ${ESC(AGE(d.timestamp_utc))}</div>${server.online&&!server.access.managed?`<div class="access-note">${ESC(server.access.headline)}</div>`:""}</div><div class="head-metrics"><span>CPU ${ESC(d.cpu?.used_percent??"—")}%</span><span>RAM ${ESC(d.memory?.used_percent??"—")}%</span><span>Disk ${ESC(d.disk_root?.used_percent??"—")}%</span></div></header><div class="server-body">${!server.online?'<div class="stale-banner">Offline · values below are last reported data.</div>':""}${badges.length?`<div class="badges">${badges.map(x=>`<span class="help ${ESC(x.key)}" title="${ESC(`${x.label} — ${x.detail}`)}">${ESC(x.label)}</span>`).join("")}</div>`:""}${resources}${this._system(server)}${this._updates(server)}${this._services(server)}${this._docker(server)}${this._network(server)}${this._disks(server)}${temperatures}${actions}${this._manage(server)}</div></article>`;
  }
  ...
  _css(){return `:host{display:block;width:100%;max-width:none;color:var(--primary-text-color)}*{box-sizing:border-box}ha-card{display:block;width:100%;max-width:none;overflow:hidden}.wrap{width:100%;padding:20px}.card-head,.server-head,.compact-head,.compact-meta,.bar-head,.pair,.interface,.history{display:flex;justify-content:space-between;gap:10px}.card-head{align-items:flex-start;flex-wrap:wrap}h2,h3,h4{margin:0}h2{font-size:1.35rem}h3{font-size:1.1rem}.muted,small{color:var(--secondary-text-color);font-size:.8rem}.notice,.config-warning,.stale-banner{margin-top:12px;padding:10px;border-radius:8px;background:var(--secondary-background-color);font-size:.82rem}.config-warning{border-left:4px solid var(--warning-color,#f9a825)}.stale-banner{margin:0 0 12px}.mode-switch,.filters>div,.actions,.inline-actions,.service-chips{display:flex;gap:6px;flex-wrap:wrap}button,select{font:inherit;color:var(--primary-text-color);background:var(--card-background-color);border:1px solid var(--divider-color);border-radius:8px;min-height:40px;padding:7px 11px;cursor:pointer}button:hover{background:var(--secondary-background-color)}button[aria-pressed=true]{border-color:var(--primary-color);color:var(--primary-color)}button:focus-visible,select:focus-visible,summary:focus-visible{outline:2px solid var(--primary-color);outline-offset:2px}button.danger{color:var(--error-color,#c62828)}button:disabled{cursor:wait;opacity:.65}.fleet-summary{display:flex;gap:8px;margin:16px 0}.fleet-summary>div{display:flex;gap:7px;align-items:baseline;background:var(--secondary-background-color);padding:8px 12px;border-radius:9px}.fleet-summary span{font-size:.76rem;color:var(--secondary-text-color)}.filters{display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:14px}.compact-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,270px),1fr));gap:12px}.compact-server{display:flex;flex-direction:column;align-items:stretch;justify-content:flex-start;text-align:left;width:100%;height:100%;min-height:0;padding:15px;border-radius:12px;background:var(--card-background-color);border:1px solid var(--divider-color)}.compact-server.problem,.detailed-server.problem{border-left:4px solid var(--warning-color,#f9a825)}.compact-server.offline,.detailed-server.offline{border-left:4px solid var(--error-color,#c62828)}.compact-head{align-items:center}.compact-head strong{font-size:1.05rem}.compact-meta,.stale-line,.compact-footer{margin-top:5px;color:var(--secondary-text-color);font-size:.76rem}.compact-resources{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin:14px 0}.compact-resources>div{background:var(--secondary-background-color);border-radius:8px;padding:8px;text-align:center;display:grid;gap:2px}.compact-resources span{font-size:.7rem;color:var(--secondary-text-color)}.service-chips{margin:10px 0}.compact-server>small{margin-top:4px}.service-chip{padding:4px 7px;border-radius:99px;background:var(--secondary-background-color);font-size:.74rem}.service-chip.active{color:var(--success-color,#2e7d32)}.service-chip.problem{color:var(--error-color,#c62828)}.service-chip.inactive{color:var(--secondary-text-color)}.compact-footer{display:flex;justify-content:space-between;flex-wrap:wrap;gap:5px;margin-top:auto;padding-top:5px}.updates-line.ok{color:var(--success-color,#2e7d32)}.updates-line.warning{color:var(--warning-color,#b8860b)}.updates-line.critical{color:var(--error-color,#c62828)}.help{cursor:help}.detailed-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,520px),1fr));gap:14px}.compact-grid+.detailed-grid{margin-top:16px}.detailed-server{border:1px solid var(--divider-color);border-radius:12px;min-width:0;overflow:hidden}.detailed-server.selected{box-shadow:0 0 0 1px var(--primary-color)}.server-head{padding:15px;align-items:flex-start}.server-title{display:flex;gap:9px;align-items:center;flex-wrap:wrap;margin-bottom:5px}.status{font-size:.76rem;text-transform:capitalize}.status.healthy{color:var(--success-color,#2e7d32)}.status.problem{color:var(--warning-color,#b8860b)}.status.offline{color:var(--error-color,#c62828)}.access{font-size:.74rem;color:var(--secondary-text-color)}.head-metrics{display:flex;flex-direction:column;gap:4px;text-align:right;white-space:nowrap;color:var(--secondary-text-color);font-size:.72rem}.server-body{padding:0 15px 15px}.badges{display:flex;gap:5px;flex-wrap:wrap;margin-bottom:10px}.badges span{font-size:.68rem;padding:3px 6px;border-radius:5px;background:var(--warning-color,#f9a825);color:var(--text-primary-color,#fff)}.badges span.security,.badges span.oom,.badges span.disk{background:var(--error-color,#c62828)}.resources{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin-bottom:9px}.bar-head,.pair{font-size:.8rem}.track{height:7px;border-radius:9px;background:var(--divider-color);overflow:hidden;margin:5px 0}.fill{height:100%;background:var(--primary-color);border-radius:9px}.fill.warning{background:var(--warning-color,#f9a825)}.fill.critical{background:var(--error-color,#c62828)}.section{border-top:1px solid var(--divider-color);padding:9px 0}.section>summary{cursor:pointer;font-weight:600;padding:2px 0}.section-body{padding-top:10px;display:grid;gap:8px}.system-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px 18px}.pair span{color:var(--secondary-text-color)}.pair strong{text-align:right;overflow-wrap:anywhere}.facts{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.package,.service,.container{display:grid;gap:3px;border-top:1px solid var(--divider-color);padding:8px 0;font-size:.82rem}.service{grid-template-columns:1fr auto}.service small,.service .inline-actions{grid-column:1/-1}.issue{color:var(--error-color,#c62828)}.actions{margin-top:7px}.action{font-size:.78rem}.interface,.history{font-size:.8rem;flex-wrap:wrap}.interface small{width:100%}.filesystem{padding-bottom:7px}.filesystem>.bar-row{margin-top:5px}.nested summary{cursor:pointer}.package-list{max-height:280px;overflow:auto}.manage-group{display:grid;gap:2px}.manage-group h4{font-size:.8rem;color:var(--secondary-text-color)}.read-only{margin:0 0 4px;padding:10px;border-radius:8px;background:var(--secondary-background-color);border-left:4px solid var(--warning-color,#f9a825);color:var(--primary-text-color);font-size:.84rem}.read-only p{margin:4px 0 0;color:var(--secondary-text-color)}.read-only code{display:block;margin-top:3px;font-size:.8rem;overflow-wrap:anywhere;color:var(--primary-text-color)}.access-note{margin-top:4px;font-size:.76rem;color:var(--warning-color,#b8860b)}.empty{padding:20px 0}.confirm-dialog{border:1px solid var(--divider-color);border-radius:12px;background:var(--card-background-color);color:var(--primary-text-color);max-width:min(90vw,430px);padding:20px;box-shadow:0 8px 24px #0004}.confirm-dialog::backdrop{background:#0009}.confirm-dialog h3{margin-bottom:8px}.confirm-label{display:grid;gap:6px}.confirm-label input{font:inherit;padding:10px;background:var(--secondary-background-color);color:var(--primary-text-color);border:1px solid var(--divider-color);border-radius:8px}.modal-actions{display:flex;justify-content:flex-end;gap:8px;margin-top:18px}@media(max-width:700px){.wrap{padding:12px}.detailed-grid,.compact-grid{grid-template-columns:1fr}.server-head{padding:12px}.server-body{padding:0 12px 12px}.system-grid{grid-template-columns:1fr}.resources{gap:8px}.mode-switch{width:100%}.mode-switch button{flex:1}}`;}
}
